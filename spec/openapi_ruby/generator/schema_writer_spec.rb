...
RSpec.describe OpenapiRuby::Generator::SchemaWriter do
  let(:output_dir) { File.join(Dir.tmpdir, "openapi_ruby_test_#{SecureRandom.hex(4)}") }
  ...
  describe "#write!" do
    it "generates a YAML spec file" do
      ctx = OpenapiRuby::DSL::Context.new("/health", schema_name: :public_api)
      ctx.get("Health check") { response(200, "OK") }
      OpenapiRuby::DSL::MetadataStore.register(ctx)

      writer = described_class.new(:public_api, OpenapiRuby.configuration.schemas[:public_api])
      path = writer.write!
    ...
    it "includes components from registry" do
      ...
      ctx = OpenapiRuby::DSL::Context.new("/test", schema_name: :public_api)
  ...
  describe "#build_document" do
    it "merges paths from multiple contexts" do
      ctx1 = OpenapiRuby::DSL::Context.new("/users", schema_name: :public_api)
      ctx1.get("List") { response(200, "OK") }
      ctx2 = OpenapiRuby::DSL::Context.new("/posts", schema_name: :public_api)
      ctx2.get("List") { response(200, "OK") }

      OpenapiRuby::DSL::MetadataStore.register(ctx1)
      OpenapiRuby::DSL::MetadataStore.register(ctx2)
  ...
  describe ".generate_all!" do
    ...
    it "warns when no schemas configured" do
      OpenapiRuby.configuration.schemas = {}
